...
namespace SoundBanks_Editor
{
    public static class TreeNodeFunctions
    {
        ...
        internal static void TreeNodeDeleteNode(TreeView TreeViewFile, TreeNode Name, string NodeTag, ProjectFile FileProperties)
        {
        ...
        internal static void TreeNodeAddNewNode(string Parent, string n_Name, string DisplayName, int SelectedImageIndex, int ImageIndex, string Tag, Color TextColor, TreeView TreeViewToEdit, ProjectFile FileProperties)
        {
        ...
        internal static TreeNode FindRootNode(TreeNode treeNode)
        {
            ...
            while (treeNode.Parent != null && treeNode.Tag.Equals("Root"))
            {
                RootNode = treeNode.Parent;
            }

            return RootNode;
        }

        internal static bool CheckIfNodeExists(TreeView SearchControl, string Name, Dictionary<int, EXSound> SoundsList, Dictionary<string, EXAudio> AudiosList)
        {
            bool Exists = false;
            string Key;

            Key = EXObjectsFunctions.GetSoundKeyByName(SoundsList, Name);
            if (string.IsNullOrEmpty(Key))
            {
                Key = EXObjectsFunctions.GetAudioKeyByName(AudiosList, Name);
            }

            TreeNode[] nod = SearchControl.Nodes.Find(Key, true);
            if (nod.Length > 0)
            {
                Exists = true;
            }
