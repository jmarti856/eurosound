    ...
    public static class YamlReader
    {
        ...
        internal static void ReadYamlFile(Dictionary<int, EXSound> SoundsList, Dictionary<string, EXAudio> AudioDict, TreeView TreeViewControl, string FilePath, string SoundName, string SoundHashcode, bool ShowResultsAtEnd, ProjectFile FileProperties, ResourceManager ResxM)
        {
            ...
            if (FileCheck.Equals("#ftype:2"))
            {
                ...
                if (!TreeNodeFunctions.CheckIfNodeExists(TreeViewControl, SoundName, SoundsList, AudioDict))
                {
                    /*--Add Sound--*/
                    ...
                    foreach (KeyValuePair<int, int[]> Entry in SamplesProperties)
                    {
                        ...
                        if (Entry.Value[0] < 0)
                        {
                            if (!SoundNodeAdded)
                            {
                                TreeNodeFunctions.TreeNodeAddNewNode("StreamedSounds", SoundID.ToString(), SoundName, 2, 2, "Sound", Color.Black, TreeViewControl, FileProperties);
                        ...
                        else
                        {
                            if (!SoundNodeAdded)
                            {
                                TreeNodeFunctions.TreeNodeAddNewNode("Sounds", SoundID.ToString(), SoundName, 2, 2, "Sound", Color.Black, TreeViewControl, FileProperties);
                                SoundNodeAdded = true;
                            }
                            ...
                            else
                            {
                                ...
                                if (File.Exists(AudioPath))
                                {
                                    ...
                                    string MD5AudioFilehash = EXObjectsFunctions.LoadAudioAddToListAndTreeNode(AudioPath, SampleName, AudioDict, TreeViewControl, AudioProps, FileProperties);
                        ...
                        TreeNodeFunctions.TreeNodeAddNewNode(SoundID.ToString(), EXObjectsFunctions.RemoveWhiteSpaces(SampleName), SampleName, 4, 4, "Sample", Color.Black, TreeViewControl, FileProperties);
                    }
                }
                else
                {
