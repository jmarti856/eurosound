    ...
    internal class MarkerFiles_Exporter
    {
        internal void ExportMarkersFromSound(string FilePath, string SoundName, EXSoundStream SoundToExport)
        {
            using (StreamWriter TextFileWriter = File.CreateText(FilePath))
            {
                ...
                TextFileWriter.WriteLine("*STRSTARTMARKERS " + SoundToExport.StartMarkers.Count + " {");
                foreach (EXStreamStartMarker StartMarker in SoundToExport.StartMarkers)
                {
                ...
                TextFileWriter.WriteLine("}");
                TextFileWriter.WriteLine("");
                TextFileWriter.WriteLine("*STRMARKERS " + SoundToExport.Markers.Count + " {");
                foreach (EXStreamMarker Marker in SoundToExport.Markers)
                {
                    TextFileWriter.WriteLine("\t*MARKER {");
