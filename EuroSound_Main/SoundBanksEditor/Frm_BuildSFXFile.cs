...
namespace EuroSound_Application
{
    public partial class Frm_BuildSFXFile : Form
    {
        ...
        private void Frm_BuildSFXFile_Load(object sender, EventArgs e)
        {
            ...
            if (BackgroundWorker_BuildSFX.IsBusy != true)
            {
        ...
        private void BackgroundWorker_BuildSFX_DoWork(object sender, DoWorkEventArgs e)
        {
            ...
            BinaryWriter BWriter = new BinaryWriter(File.Open(@"X:\Sphinx\Binary\_bin_PC\\" + FileName + ".SFX", FileMode.Create, FileAccess.Write), Encoding.ASCII);
        ...
        private void BackgroundWorker_BuildSFX_RunWorkerCompleted(object sender, RunWorkerCompletedEventArgs e)
        {
            if (e.Cancelled)
            {
                MessageBox.Show("Output cancelled by user", "EuroSound", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else if (e.Error != null)
            {
                MessageBox.Show("An error occurred in the output and the file might be corrupt", "EuroSound", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            else
            {
                MessageBox.Show("Outputed finished successfully", "EuroSound", MessageBoxButtons.OK, MessageBoxIcon.Information);
            }
            this.Close();
            this.Dispose();
        }
